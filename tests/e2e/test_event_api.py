...
class TestEventAPIE2E:
    ...
    @pytest.mark.asyncio
    async def test_register_attendee_duplicate_registration(self, async_client: AsyncClient):
    ...
    async def test_register_attendee_capacity_exceeded(self, async_client: AsyncClient):
        ...
        response = await async_client.post(f"/events/{event_id}/register", json=attendee2_data)
    ...
    async def test_get_event_attendees_with_pagination(self, async_client: AsyncClient):
        ...
        # Act - Test pagination
        response = await async_client.get(f"/events/{event_id}/attendees?limit=3&offset=0")
    ...
    async def test_full_event_workflow(self, async_client: AsyncClient):
        ...
        for attendee_data in attendees_data:
            registration_response = await async_client.post(f"/events/{event_id}/register", json=attendee_data)
            assert registration_response.status_code == 201
        
        # Step 3: Verify event is now full
        fourth_attendee = {"name": "David Wilson", "email": "david@example.com"}
        full_response = await async_client.post(f"/events/{event_id}/register", json=fourth_attendee)
    ...
    @pytest.mark.asyncio
    async def test_get_events_timezone_conversion_accuracy(self, async_client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_get_events_with_pagination_and_timezone(self, async_client: AsyncClient):
        ...
        response = await async_client.get("/events/?limit=2&offset=0&timezone=US/Eastern")
