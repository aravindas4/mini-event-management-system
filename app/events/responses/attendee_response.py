...
class AttendeeListResponse(BaseModel):
    ...
    @classmethod
    def from_domain_list(cls, attendees: list[Attendee], total_count: int, limit: int, offset: int) -> "AttendeeListResponse":
        """Create response from domain model list"""
        return cls(
            attendees=[AttendeeResponse.from_domain(attendee) for attendee in attendees],
